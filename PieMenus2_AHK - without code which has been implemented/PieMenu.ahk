...
;Read Json Settings file to object
	...
	Try 
	{
		FileRead, markingMenus, %A_ScriptDir%\markingMenus.json
		global markingMenus := Json.Load(markingMenus)
	} catch e {
		msgbox, % "markingMenus file is invalid JSON.`n`nNo Pie Menus for you :(`n`nFix settings file at:`n" . A_ScriptDir . "\settings.json"
		Exitapp
	}
	...
	If (!WinActive("ahk_group regApps"))
		...
		for menus in settings.appProfiles[1].pieMenus
			...
			if (settings.appProfiles[1].pieMenus[menus].hotkey == activePieKey)
				{
				; MsgBox % GetKeyState(activePieKey, "P")
				; blockBareKeys(activePieKey, hotKeyArray, 1)			
				funcAddress := runPieMenu(1, menus, False)
				pieLaunchedState := 0		
			...
			if (testAHKHandle == activeWinProc)
				...
				for menus in settings.appProfiles[profiles].pieMenus
					...
					if settings.appProfiles[profiles].pieMenus[menus].hotkey == activePieKey
						{
							; blockBareKeys(activePieKey, hotKeyArray, 1)
							funcAddress := runPieMenu(profiles, menus, False)
							pieLaunchedState := 0
							; blockBareKeys(activePieKey, hotKeyArray, 0)
							activePieKey := ""
							runPieFunction(funcAddress)
...
;Block RButton when Pie Menu is opened
#If (pieLaunchedState == 1)
{
	RButton::Return
}

#Include %A_ScriptDir%\lib\BlockKeys.ahk
