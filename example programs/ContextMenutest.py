...
class ContextMenu(QtWidgets.QMenu):
    ...
    def initAnimation(self):
        ...
        self.geomAnim = QtCore.QPropertyAnimation(self, b'geometry', easingCurve=QtCore.QEasingCurve.OutCubic, duration=200)
    ...
    def add_action(self, *args, custom_css = None):
        if custom_css:
            action = self.addAction(*args).hovered.connect(lambda: self.update_css(custom_css))
        else:
            action = self.addAction(*args).hovered.connect(self.reset_css)
